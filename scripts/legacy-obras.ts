/**
 * Dados das obras como existiam ANTES da migração para o Sanity (agosto/2026).
 *
 * Vieram de src/lib/constants.ts e ficam aqui só para `migrate-obras.ts`
 * continuar rodando e para registrar de onde o conteúdo do painel veio.
 * O site NÃO lê este arquivo — as obras vêm do Sanity (src/sanity/queries.ts).
 * Não editar: mudanças aqui não aparecem em lugar nenhum.
 */
